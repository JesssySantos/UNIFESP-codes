...
void Inserir(FilaC *f,int N){
    ...
    if (f->ultimo + 1 != N){
        ...
        for(i=0;i<f->ultimo+2;i++){
            j=i+1;
            if(f->vendas[i].tempo < f->vendas[j].tempo){
                    p_menor = i;
                    free(f->vendas[p_menor].tempo);
                    printf("\nRemovido item do funcionario %d\n",f->vendas[p_menor].ID);
                    scanf("%d",&time);
                    f->vendas[p_menor].tempo = time;
                    f->vendas[p_menor].num_ligacoes++;
                }
                else if(f->vendas[i].tempo <= f->vendas[j].tempo && i==f->ultimo){
                    p_menor = i;
                    free(f->vendas[p_menor].tempo);
                    printf("\nRemovido item do funcionario %d\n",f->vendas[p_menor].ID);
                    scanf("%d",&time);
